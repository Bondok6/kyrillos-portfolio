...
const SkillsAnimation = () => {
  useEffect(() => {
    (function () {
      function SVG3DTagCloud(element, params) {
        ...
        function reInit() {
          ...
          if (
            settings.width.toString().indexOf('%') > 0 ||
            settings.height.toString().indexOf('%') > 0
          ) {
            svgWidth = Math.round(
              (element.offsetWidth / 100) * parseInt(settings.width)
            );
            svgHeight = Math.round(
              (svgWidth / 100) * parseInt(settings.height)
            );
          } else {
        ...
        function render() {
          ...
          for (var i = 0, l = entryHolder.length; i < l; i++) {
            ...
            if (mouseReact) {
              ...
              if (entry.mouseOver) {
                opacity +=
                  (settings.opacityOver - opacity) / settings.opacitySpeed;
              } else {
                opacity +=
                  (settings.opacityOut - opacity) / settings.opacitySpeed;
              }
            }
        ...
        $.fn.svg3DTagCloud = function (params) {
          ...
          return this.each(function () {
            if (!$.data(this, 'plugin_SVG3DTagCloud')) {
              $.data(
                this,
                'plugin_SVG3DTagCloud',
                new SVG3DTagCloud(this, params)
              );
            } else {
              ...
              if (plugin[params]) {
                ...
                $.error(
                  'Method ' + params + ' does not exist on jQuery.svg3DTagCloud'
                );
              }
            }
          });
    ...
    $(document).ready(function () {
      ...
      // settings on medium screens
      if (window.innerWidth < 1200 && window.innerWidth > 768) {
        settings.width = 400;
        settings.height = 400;
        settings.raduis = '70%';
        settings.raduisMin = 50;
        settings.fontSize = '1.5rem';
      }
